...
Public gift data contains its opaque ID, emoji ID, displayed author name, server-created date, accepted pose, revision, and public text when applicable. Private text never appears in the page HTML, hydration data, public snapshots, WebSocket events, or a shared cache.
...
Records include gifts, optional messages, accepted object poses, a monotonic house revision, rate limits, and creation receipts. Identity credentials live in separate visitor auth storage. Original portfolio objects and visitor gifts have distinct kinds, so a reclaim command cannot delete a built-in object. Gift creation and deletion are transactional. Creation receipts retain a payload hash after withdrawal so retrying a lost response cannot resurrect a removed gift or retain its deleted message.

All visitors can rearrange the house, consistent with the earlier concept. For the first version, synchronize accepted final placements after release; do not stream movement during dragging. New gifts and withdrawals propagate to other connected visitors. Presence counts and cursors remain deferred.

The canonical scene starts at 500 × 600 and grows with the pile; a scrollable viewport preserves access to gifts as it grows. The client sends only the released object's pose and the revision at pickup. The server settles the canonical Matter world in a bounded simulation, persists the result, then broadcasts it. Clients apply these authoritative poses without continuing an independent simulation. A stale revision returns a conflict and the current scene; it cannot overwrite another visitor's accepted rearrangement. No continuous server physics loop runs.

Hibernation WebSockets are a suitable transport for public scene events. Authenticate commands server-side; any session establishment must keep browser secrets out of query strings. Send private message bodies through their separate authorized endpoint, not the shared room broadcast. [Cloudflare WebSockets](https://developers.cloudflare.com/durable-objects/best-practices/websockets/).
...
| Shared movement | Synchronize final placement on release for the first version |
...
4. Connect the selected concurrency behavior, coordinate mapping, reconnect snapshots, and conflict handling. Implement the chosen lifecycle and moderation policy.
...
Required cases include duplicate-submit retries, two visitors editing the same object, gift withdrawal while another visitor reads it, identity persistence across reloads/tabs, stale suggestion responses, disconnected reconnection, and attempts to reclaim another browser's gift or read its private message. Validate the existing CMS and scheduled handler after any deployment integration change.
